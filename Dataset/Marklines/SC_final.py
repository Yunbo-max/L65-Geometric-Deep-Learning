# -*- coding: utf-8 -*-
# @Author: Yunbo
# @Date:   2024-02-27 14:28:32
# @Last Modified by:   Yunbo
# @Last Modified time: 2024-02-27 14:36:39
...
def calculate_eigenvalues_eigenvectors(adjacency_matrices):
    ...
    for adj_matrix in adjacency_matrices:
        ...
        degree_matrix = np.diag(np.sum(adj_matrix, axis=1)) # Error line
...
def create_adjacency_matrices(df, num_graphs):
    ...
    for nodes in graph_nodes:
        ...
        adjacency_matrices_list.append(torch.tensor(adj_matrix))
...
# Function to save data as a .pt file
def save_as_pt_file(adjacency_matrices, eigenvalues, eigenvectors, max_eigenvalue, min_eigenvalue, additional_value_1, additional_value_2, output_file):
    num_nodes_list = [len(adj_matrix) for adj_matrix in adjacency_matrices]
    torch.save((adjacency_matrices, eigenvalues, eigenvectors, num_nodes_list, torch.tensor(max_eigenvalue), torch.tensor(min_eigenvalue), additional_value_1, additional_value_2), output_file)
...
def main(csv_file, num_graphs, output_file):
    ...
    max_eigenvalue = max(max(ev) for ev in eigenvalues)
    min_eigenvalue = min(min(ev) for ev in eigenvalues)
    additional_value_1 = False
    additional_value_2 = max(len(adj_matrix) for adj_matrix in adjacency_matrices)
    save_as_pt_file(adjacency_matrices, eigenvalues, eigenvectors, max_eigenvalue, min_eigenvalue, additional_value_1, additional_value_2, output_file)
